...
The GitHub Trademarks Logo. They published great guidelines for using its logo
and trademarks and even supplied the preferred `.svg` material. However, the
logo file is not geometrically matching and the coloration is badly coded. The
Octocat silhouette is a missing piece instead of a filler.

While `.svg` has non-destructive scale up & down properties and can be converted
into any rasterized images (e.g. `jpeg`, `webp`, `avif`, etc). Broken codes
means a re-draw is required.

This project re-draws GitHub Trademarks Logo specifically for 1200x630 size.
...
Please read through GitHub's Trademark Engineering Specification
(https://brand.github.com/) before editing to avoid any unwanted legal
implications.
...
> **YOU ARE STILL OBLIGATED TO COMPLY WITH GITHUB TRADEMARK REQUIREMENTS
> (Read: https://brand.github.com/)**.
>
